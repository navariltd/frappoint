...
def invalidate_counter(
    counter_date, resource_type: str, resource_reference: str
) -> None:
...
def lock_counter_resource_rows(
    ...
    if resources:
        ...
        resource_types = (
            [resource_type] if resource_type else list(COUNTER_RESOURCE_DOCTYPES)
        )
        for r_type in resource_types:
            doctype = COUNTER_RESOURCE_DOCTYPES.get(r_type)
            if not doctype or not frappe.db.table_exists(doctype):
                continue
            targets[r_type].update(
                frappe.get_all(doctype, pluck="name", order_by="name asc")
            )
...
def get_available_slots(
    ...
    duration = cint(
        required_duration_minutes or service_type.default_duration_in_minutes or 0
    )
    ...
    if requires_unit:
        units = frappe.get_all(
            "Service Unit", filters=unit_filters, fields=["name", "unit_name"]
        )
    ...
    if exclude_appointment_id:
        ...
        if excluded_capacity:
            ...
            provider_slot_map = _build_available_slot_map(provider_counters)
            unit_slot_map = (
                _build_available_slot_map(unit_counters) if requires_unit else {}
            )
        ...
        for date_key, slot_times in provider_slot_map.get(provider, {}).items():
            ...
            for start_slot in windows:
                ...
                if requires_unit:
                    ...
                    unit_ids_for_window = (
                        matching_unit_ids
                        if include_all_service_units
                        else matching_unit_ids[:1]
                    )
                    ...
                    results.append(
                        {
                            ...
                            "service_unit_name": (
                                unit_names.get(unit_id) if unit_id else None
                            ),
                            "date": date_key,
    ...
    results.sort(
        key=lambda row: (row["date"], row["start_time"], row.get("provider_name") or "")
    )
    return results
...
def _get_resource_remaining_capacity_map(
    ...
    for resource_type, resource_ids in resources:
        ...
        for row in rows:
            ...
            capacity_by_slot[key] = max(
                capacity_by_slot.get(key, 0.0), available_capacity
            )
...
def _reserved_counter_slot_times(
    row: dict[str, Any], slot_size_minutes: int
) -> set[time]:
    ...
    reserve_start = customer_start - timedelta(
        minutes=cint(row.get("buffer_before") or 0)
    )
...
def _build_shift_capacity_slots(
    ...
    if resource_type:
        assignments = [
            ...
            if (
                (
                    resource_type == "Service Provider"
                    and row.provider == resource_reference
                )
                or (
                    resource_type == "Service Unit"
                    and row.service_unit == resource_reference
                )
            )
        ]
    ...
    provider_active = set(
        frappe.get_all("Service Provider", filters={"active": 1}, pluck="name")
    )
    unit_rows = {
        row["name"]: row
        for row in frappe.get_all(
    ...
    for assignment in assignments:
        ...
        for slot_time, is_break in _build_shift_slot_sequence(
            shift, target_date, slot_size_minutes
        ):
...
def _build_excluded_appointment_capacity_map(
    ...
    if not appointment_name or not frappe.db.table_exists(
        "Service Resource Allocation"
    ):
        return {}
...
def _delete_existing_counters(
    target_date, resource_type: str | None, resource_reference: str | None
) -> None:
...
def _interval_to_slot_times(
    target_date, start_time, end_time, slot_size_minutes: int
) -> list[str]:
...
def _service_type_requires_unit(service_type_id: str) -> tuple[bool, list[str]]:
    ...
    unit_types = [
        row["service_unit_type"] for row in rows if row.get("service_unit_type")
    ]
...
def _get_service_providers(service_type_id: str, provider_id: str | None) -> list[str]:
    if provider_id:
        ...
        is_active = frappe.db.exists(
            "Service Provider", {"name": provider_id, "active": 1}
        )
...
def _build_available_slot_map(
    counter_rows: list[dict[str, Any]],
) -> dict[str, dict[Any, list[time]]]:
    resource_date_slots: dict[str, dict[Any, list[time]]] = defaultdict(
    ...
    for resource in resource_date_slots:
        for date_key in resource_date_slots[resource]:
            resource_date_slots[resource][date_key] = sorted(
                set(resource_date_slots[resource][date_key])
            )
    ...
    for assignment in assignments:
        ...
        while current <= assignment_end:
            if _assignment_matches_date(assignment, current):
                for slot_time, is_break in _build_shift_slot_sequence(
                    shift, current, slot_size_minutes
                ):
                    if is_break:
                        continue
                    provider_unit_slots[assignment.provider][current][service_unit].add(
                        get_time(slot_time)
                    )
            current = add_days(current, 1)
