...
def reserve_and_create_allocations(
    ...
    if not frappe.db.exists("Service Appointment", appointment_name):
        raise CapacityReservationError(
            _("Service Appointment {0} not found").format(appointment_name)
        )
...
def _release_capacity_for_allocation_scope(
    ...
    if target_status not in RELEASE_ALLOCATION_STATUSES:
        raise CapacityReservationError(
            _("Unsupported released allocation status: {0}").format(target_status)
        )
    filters: dict[str, Any] = {
        "allocation_status": ["in", list(ACTIVE_RESERVATION_STATUSES)]
    }
    if allocation_names:
    ...
    try:
        ...
        if not appointment_names:
            locked_appointment_names = sorted(
                {
                    row.get("service_appointment")
                    for row in candidate_rows
                    if row.get("service_appointment")
                }
            )
            _lock_service_appointment_rows(locked_appointment_names)
        candidate_names = sorted(
            {row.get("name") for row in candidate_rows if row.get("name")}
        )
...
def confirm_held_allocations(appointment_name: str, commit: bool = False) -> int:
    ...
    if frappe.db.get_value(
        ...
        raise CapacityReservationError(
            _(
                "Couple appointments must confirm capacity through the atomic pair operation"
            )
        )
...
def confirm_couple_held_allocations(
    ...
    names = _validate_couple_appointment_names(appointment_names)
    return _confirm_held_allocations_for_appointments(
        names, commit=commit, savepoint_prefix="confirm_couple"
    )
...
def _confirm_held_allocations_for_appointments(
    ...
    try:
        ...
        candidate_names = frappe.get_all(
            "Service Resource Allocation", filters=filters, pluck="name"
        )
...
def _validate_couple_ledger_rows(
    appointment_names: list[str], rows: list[dict[str, Any]]
) -> None:
    for appointment_name in appointment_names:
        record = _get_appointment_couple_record(appointment_name)
        expected = sorted(
            _allocation_signature(row)
            for row in _expected_couple_allocation_rows(record, appointment_name)
        )
        actual = sorted(
        ...
        if actual != expected:
            raise CapacityReservationError(
                _("Held allocations are incomplete for couple appointment {0}").format(
                    appointment_name
                )
            )
...
def _validate_couple_reservation_requests(
    ...
    if not isinstance(appointments, list | tuple) or len(appointments) != 2:
        raise CapacityReservationError(
            _("A couple reservation requires exactly two appointments")
        )
    ...
    for request in appointments:
        if not isinstance(request, dict) or not request.get("appointment_name"):
            raise CapacityReservationError(
                _("Each couple reservation requires an appointment_name")
            )
    ...
    if len(booking_ids) != 1 or not next(iter(booking_ids), None):
        raise CapacityReservationError(
            _("Couple appointments must share the same booking ID")
        )

    appointment_dates = {
        getdate(record.get("appointment_date"))
        for record in appointment_records.values()
    }
    start_times = {
        _time_str(record.get("start_time")) for record in appointment_records.values()
    }
    if len(appointment_dates) != 1 or len(start_times) != 1:
        raise CapacityReservationError(
            _("Couple appointments must share the same start time")
        )

    primary_count = sum(
        cint(record.get("is_primary_in_couple"))
        for record in appointment_records.values()
    )
    if primary_count != 1:
        raise CapacityReservationError(
            _("Couple appointments must identify exactly one primary appointment")
        )

    service_units = [
        record.get("service_unit") for record in appointment_records.values()
    ]
    if any(unit is None for unit in service_units) or len(set(service_units)) != 1:
        raise CapacityReservationError(
            _("Couple appointments must share the same service unit")
        )
    shared_unit = service_units[0]
    unit = frappe.db.get_value(
        "Service Unit", shared_unit, ["allow_overlap", "capacity"], as_dict=True
    )
    if (
        not unit
        or not cint(unit.get("allow_overlap"))
        or flt(unit.get("capacity") or 1) < 2
    ):
        raise CapacityReservationError(
            _(
                "Shared couple service unit {0} does not allow two simultaneous services"
            ).format(shared_unit)
        )
    ...
    for raw_request, appointment_name in zip(appointments, names, strict=True):
        ...
        if not isinstance(allocations, list | tuple) or not allocations:
            raise CapacityReservationError(
                _("Couple appointment {0} requires allocation payloads").format(
                    appointment_name
                )
            )
        if not any(
            row.get("resource_type") == "Service Provider" for row in allocations
        ):
            raise CapacityReservationError(
                _("Couple appointment {0} requires a provider allocation").format(
                    appointment_name
                )
            )
        ...
        if booking_name != record.get("booking_id"):
            raise CapacityReservationError(
                _("Allocation booking must match appointment {0}'s booking ID").format(
                    appointment_name
                )
            )
    ...
    if len(statuses) != 1:
        raise CapacityReservationError(
            _("Both couple appointments must use the same allocation status")
        )
...
def _validate_couple_appointment_names_with_records(
    ...
    if len(names) != 2 or len(set(names)) != 2:
        raise CapacityReservationError(
            _("Couple appointment names must be present and distinct")
        )

    for name in names:
        if not frappe.db.exists("Service Appointment", name):
            raise CapacityReservationError(
                _("Service Appointment {0} not found").format(name)
            )
    ...
    for request in requests:
        ...
        for row in request["prepared_allocations"]:
            if row["allocation_date"] != expected_date:
                raise CapacityReservationError(
                    _("Allocation date must match couple appointment {0}").format(
            ...
            if row["appointment_end_time"] != expected_end:
                raise CapacityReservationError(
                    _("Allocation end must match couple appointment {0}").format(
                        request["appointment_name"]
                    )
                )

        expected_rows = _expected_couple_allocation_rows(
            record, request["appointment_name"]
        )
        actual_signatures = sorted(
            _allocation_signature(row) for row in request["prepared_allocations"]
        )
        expected_signatures = sorted(
            _allocation_signature(row) for row in expected_rows
        )
        if actual_signatures != expected_signatures:
            raise CapacityReservationError(
                _(
                    "Allocation resources, buffers, and capacity must match couple appointment {0}"
                ).format(request["appointment_name"])
            )


def _expected_couple_allocation_rows(
    record: dict[str, Any], appointment_name: str
) -> list[dict[str, Any]]:
    ...
    if not service_type or not provider:
        raise CapacityReservationError(
            _("Couple appointment {0} requires a service and provider").format(
                appointment_name
            )
        )
    if not frappe.db.exists(
        ...
        raise CapacityReservationError(
            _(
                "The assigned provider cannot offer couple appointment {0}'s service"
    ...
    if not required_unit_types and service_unit:
        raise CapacityReservationError(
            _("Couple appointment {0}'s service does not use a service unit").format(
                appointment_name
            )
        )
    if service_unit:
        ...
        if (
            ...
            raise CapacityReservationError(
                _("Service unit is not eligible for couple appointment {0}").format(
                    appointment_name
                )
            )
    ...
    appointment_start = datetime.combine(
        allocation_date, get_time(record.get("start_time"))
    )
    appointment_end = datetime.combine(
        allocation_date, get_time(record.get("end_time"))
    )
    ...
    if allocation_start.date() != allocation_date or (
        ...
        raise CapacityReservationError(
            _("Couple appointment buffers cannot cross calendar dates")
        )
...
def _prepare_allocation_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if missing:
        raise CapacityReservationError(
            _("Missing allocation fields: {0}").format(", ".join(missing))
        )
...
def _ensure_counter_rows(prepared: list[dict[str, Any]]) -> None:
    ...
    pairs = {
        (p["allocation_date"], p["resource_type"], p["resource_reference"])
        for p in prepared
    }
...
def _apply_counter_deltas(prepared: list[dict[str, Any]], direction: str) -> None:
    ...
    if direction not in ("reserve", "release"):
        raise CapacityReservationError(
            _("Unsupported counter delta direction: {0}").format(direction)
        )
...
def _update_appointment_allocation_status(appointment_name: str, status: str) -> None:
    if frappe.db.exists(
        "DocField", {"parent": "Service Appointment", "fieldname": "allocation_status"}
    ):
        frappe.db.set_value(
            "Service Appointment",
            appointment_name,
