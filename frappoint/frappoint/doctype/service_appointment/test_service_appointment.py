...
class TestServiceAppointment(TestCase):
    ...
    def test_couple_confirmation_savepoint_precedes_first_submit(self):
        ...
        caller = SimpleNamespace(
            name=primary.name, couple_appointment_id=secondary.name
        )
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, "linked submit failed"):
                ServiceAppointment._confirm_couple_appointments(
                    caller, savepoint="outer_pair_submit"
                )
    ...
    def test_pair_insert_can_defer_calendar_and_resource_side_effects(self):
        appointment = SimpleNamespace(
            flags=SimpleNamespace(
                skip_calendar_event=True, skip_resource_allocation=True
            ),
    ...
    def test_couple_configuration_allows_same_unit_with_individual_end_times_and_providers(
        self,
    ):
        appointment = SimpleNamespace(
            name="APT-PRIMARY",
    ...
    def test_cancelling_appointment_with_submitted_payment_shows_actionable_message(
        self,
    ):
        appointment = SimpleNamespace(
            name="TEST-APPOINTMENT", docstatus=1, status="Confirmed"
        )

        with (
            patch.object(frappe, "get_doc", return_value=appointment),
            patch.object(
                frappe, "db", SimpleNamespace(exists=lambda *args, **kwargs: "PAY-0001")
            ),
            patch(
                "frappoint.frappoint.doctype.service_appointment.service_appointment._",
                side_effect=lambda message: message,
