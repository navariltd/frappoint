	...
	<div class="relative">
		...
		>
			<FeatherIcon
				...
				:class="selectedSort ? 'text-primary' : 'text-gray-400 group-hover:text-primary'"
				name="bar-chart-2"
			/>
			<span
			...
			>
				...
				>
					<FeatherIcon
						...
						name="check"
					/>
					<span :class="{ 'ml-6': selectedSort !== option.value }">
						{{ option.label }}
					</span>
				</div>
			</div>
...
const displayText = computed(() => {
	...
	const selected = props.options.find((opt) => opt.value === selectedSort.value);
